<template>
  <v-container fluid>
    ...
    <v-row>
      <v-col cols="12" class="text-center">
        <v-card class="pa-4" elevation="2">
          <v-card-title>
            <v-img :src="logo" max-width="150" class="mx-auto"></v-img>
          </v-card-title>
    ...
    <v-row class="mt-4">
      <v-col cols="12" sm="6">
        <v-file-input label="Import RH" @change="onFileChangeRH" accept=".xlsx,.xls" outlined dense
          prepend-icon="mdi-file-excel"></v-file-input>
      </v-col>
      <v-col cols="12" sm="6">
        <v-file-input label="Import Cantine" @change="onFileChangeCantine" accept=".xlsx,.xls" outlined dense
          prepend-icon="mdi-file-excel"></v-file-input>
      </v-col>
    </v-row>


    <v-row class="mt-4" v-if="!showNext">
      <v-col cols="12" class="text-center">
        <v-btn v-if="filesBoth.rh!=null && filesBoth.cantine!=null" @click="doValidate_V2" color="primary" class="ma-2">
    ...
    <v-row class="mt-4"  v-else>
      <v-col cols="12" class="text-center">
        <v-btn  @click="validateFiles" color="green" class="ma-2 " style="float: right;">
          <v-icon>mdi-check</v-icon> NEXT
        </v-btn>
      </v-col>
    </v-row>
...
export default {
  ...
  methods: {
    doValidate_V2(){
      this.importFile(this.filesBoth.rh, 'RH');
      
      
      //this.sendDataToBackend();
    },
    onFileChangeRH(event) {
      this.filesBoth.rh=event;
      //this.importFile(event, 'RH');
    },
    onFileChangeCantine(event) {
      this.filesBoth.cantine=event;
    ...
    async sendDataToBackend() {
      try {
        ...
        const existingConsumersResponse = await axios.get('http://10.20.33.112:9017/api/consommateurs');
        ...
        const newConsumers = this.cantineTableData
          .slice(1)
          .filter((e) => {
            return e.username != null && e.username != ""
        ...
        if (newConsumers.length > 0) {
          await axios.post('http://10.20.33.112:9017/api/consommateurs/store', { consumers: newConsumers }, {
            headers: {
    ...
    async sendAnomaliesToBackend() {
      try {
        ...
        await axios.post('http://10.20.33.112:9017/api/consommations/store', { anomalies }, {
      ...
      if (file) {
        ...
        reader.onload = (e) => {
          ...
          if (type === 'RH') {
            ...
            this.rhTableData = jsonData
            .filter((row)=>{
              return row[0] && row[0]!=null && row[0]!=""
            })
            .slice(1)
            .map(row => row.map(cell => formatDate(cell)));
        ...
        if(type=="RH"){
          this.importFile(this.filesBoth.cantine, 'Cantine');
        }
        else {
          this.showNext=true;
          //this.validateFiles();
    ...
    validateFiles() {
      ...
      cantineData.forEach(cantineEntry => {
        ...
        // Si l'heure est entre 00:00 et 07:00
        if (this.parseTime(cantineEntry.heure) >= this.parseTime("00:00") &&
            this.parseTime(cantineEntry.heure) <= this.parseTime("06:00")    ) {
              console.log("mat :",)
